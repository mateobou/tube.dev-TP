document.querySelector('form').addEventListener('submit', function (event) {
  ...
  xhr.send({firstName:formData.firstName,lastName:formData.lastName,password:formData.password,email:formData.email}); // envoie la requête avec les données du formulaire
  xhr.onload = function () {
    if (xhr.status === 200) {
